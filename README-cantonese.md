...
- **零依賴環境**：唔需要手動安裝 Python。第一次執行時會自動下載同更新最新嘅 `yt-dlp.exe` 同 `ffmpeg.exe`。
- **智慧剪貼簿偵測**：開啟後自動讀取剪貼簿中嘅影片網址，按個 Enter 就可以直接下載，懶人福音。
- **多平台支援**：完美支援 YouTube（包括播放清單、Shorts）、Bilibili、TikTok、X (Twitter)、Facebook 等主流平台。
- **豐富嘅格式選擇**：
  - 支援自動最高畫質（4K/2K/1080p）合併，並自動內嵌字幕同封面。
  - 提供 1080p、720p 限制選項。
  - 支援高音質純音樂提取（MP3 320k / M4A）同完整播放清單打包。
- **進階功能**：
  - 支援 `cookies.txt` 以下載會員或受限制影片。
  - 自動偵測並啟用本地或系統嘅 Node.js / Deno 執行環境（用於繞過複雜嘅反爬蟲驗證）。
  - 支援自訂協議喚起同命令列快速參數。
...
2. 雙擊執行 `dl.bat`。
3. 程式會自動檢查並下載必要嘅核心工具（`yt-dlp` 同 `ffmpeg`）。
